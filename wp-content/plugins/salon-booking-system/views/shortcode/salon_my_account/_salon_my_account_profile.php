...
foreach (SLN_Enum_CheckoutFields::toArray('customer') as $field => $label) {
    $values[$field] = !get_user_meta($current_user->ID, '_sln_' . $field, true) && isset(SLN_Enum_CheckoutFields::$additional_fields_defaults[$field])  ? SLN_Enum_CheckoutFields::$additional_fields_defaults[$field] : get_user_meta($current_user->ID, '_sln_' . $field, true) ;
}
...
<form method="post"  role="form" id="salon-my-account-profile-form" >
    ...
    <div class="container-fluid">
        <div class="row">
            <?php foreach (array_merge(SLN_Enum_CheckoutFields::toArray('defaults'),SLN_Enum_CheckoutFields::toArray('customer')) as $field => $label): ?>
