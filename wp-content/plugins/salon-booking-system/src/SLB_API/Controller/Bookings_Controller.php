...
class Bookings_Controller extends REST_Controller
{
	...
	$sql_joins = "INNER JOIN {$wpdb->prefix}postmeta pm ON p.id = pm.post_id
		    ...
		    AND DATE(pm.meta_value) >= '".date('Y-m-d', strtotime($request->get_param('start_date')))."'
		    AND DATE(pm.meta_value) <= '".date('Y-m-d', strtotime($request->get_param('end_date')))."'";
    ...
    {
	$from_time = current_time('H:i:s');
	$to_time   = date('H:i:s', current_time('timestamp') + (int)$request['hours'] * 3600);
    ...
    public function prepare_response_for_collection($booking)
    {
        ...
        $response = array(
            'id'                  => $booking->getId(),
            'created'             => strtotime($booking->getPostDate(), current_time('timestamp')),
