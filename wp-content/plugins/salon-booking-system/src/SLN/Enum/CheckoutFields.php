...
class SLN_Enum_CheckoutFields
{
    ...
    private static $customer_fields = array();
    private static $not_customer_fields = array();
    private static $requiredByDefault;
    public static $additional_fields_types = array();
    public static $additional_fields_widths = array(); 
    public static $additional_fields_defaults = array();    
    ...
    {   
        ...
        if($context == 'not-customer') {
            return self::$not_customer_fields;
        };
        return self::$fields;
    }
    ...
    public static function initAdditionalFields($additional_fields){
        ...
        foreach ($additional_fields as $field => $opts ) {
            ...
            if(!isset(self::$fields[$field])){
                ...
                if($field_settings['customer_profile']){
                    self::$customer_fields[$field] = $field_settings['label'];
                }else{
                    self::$not_customer_fields[$field] = $field_settings['label'];
                }
                ...
                self::$additional_fields_defaults[$field] = $field_settings['default'];
