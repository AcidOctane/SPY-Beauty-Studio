...
class SLN_Shortcode_SalonCalendar {
    ...
    private function prepareContentData() {
        ...
        $ret['visibility'] = isset($this->attrs['visibility']) ? $this->attrs['visibility'] : '';
        ...
        if (isset($this->attrs['days'])) {
            ...
            $ret['showDays']  = $this->attrs['days'];
        }

        $timestamp = current_time('timestamp');
        for ($i = 1; $i <= $showDays; $i++) {
            $key = date('Y-m-d', $timestamp);
            $ret['dates'][$key] = date_i18n('l', $timestamp) . ' / ' . $formatter->date($key);
			...
			array(
				'@wp_query'   => array(
					...
					'meta_query'  => array(
						...
						array(
							'key'     => '_sln_booking_date',
							'value'   => date('Y-m-d', strtotime(sprintf('+%s days', $showDays), current_time('timestamp'))),
							'type'    => 'DATE',
							'compare' => '<',
						)
					),
