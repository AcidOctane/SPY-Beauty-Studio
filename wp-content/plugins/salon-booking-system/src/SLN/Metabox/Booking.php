...
class SLN_Metabox_Booking extends SLN_Metabox_Abstract
{
    ...
    public function details_meta_box($object, $box)
    {
        ...
        echo $this->getPlugin()->loadView(
            ...
            array(
                ...
                'date' => isset($_GET['date']) ? new SLN_DateTime(sanitize_text_field(wp_unslash($_GET['date']))) : null,
                'time' => isset($_GET['time']) ? new SLN_DateTime(sanitize_text_field(wp_unslash($_GET['time']))) : null,
            )
        );
    ...
    public function save_post($post_id, $post)
    {
        ...
        if(($customer = $booking->getCustomer())){
            $fields = SLN_Enum_CheckoutFields::toArray('customer');
            if($fields){
